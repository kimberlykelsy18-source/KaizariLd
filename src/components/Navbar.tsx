...
export const Navbar = ({
  ...
  return (
    <nav className="flex items-center justify-between px-6 py-4 bg-white shadow-md">
      <div
        className="text-xl font-bold text-[#005a7c] cursor-pointer"
        onClick={() => onNavigate?.('home')}
      >
        Kaizari L&D International
      </div>
      <ul className="flex space-x-6">
        {menuStructure.map((menu) => (
          <li key={menu.id} className="relative group cursor-pointer">
            <span
              className={`text-gray-700 hover:text-[#005a7c] transition ${
                activePage === menu.id ? 'text-[#005a7c] font-semibold' : ''
              }`}
              onClick={() =>
                menu.type === 'single' && onNavigate?.(menu.id)
              }
            >
              {menu.label}
            </span>
